                    ...
                    while True:
                        ...
                        try:
                            ...
                            if "error" in message_body:
                                logging.error(f"Client {client_id} : Server error: {message_body['error']}")
                            elif "payload" in message_body:
                                # TODO: Handle binary messages (audio) placeholder
                                payload = message_body['payload']
                                if isinstance(payload, bytes):
                                    logging.info(f"Client {client_id} : [Received audio data - handling not implemented for now]")
                                logging.info(f"Client {client_id} : Server payload: {payload}")
                            # else:
                            #     print(f"Server: {message_body}")
